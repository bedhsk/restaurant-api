...
export class OrdersService {
  ...
  async updateStatus(id: string, updateStatusDto: UpdateOrderStatusDto) {
    ...
    if (updateStatusDto.status === OrderStatus.PAID || updateStatusDto.status === OrderStatus.CANCELLED) {
  ...
  async addItems(id: string, addItemsDto: AddItemsDto) {
    ...
    return this.dataSource.transaction(async (manager) => {
      // Calculate new items
      const newItemsData = this.calculateOrderItems(addItemsDto.items, products);
      ...
      const allItems = await manager.find(OrderItem, { where: { orderId: id } });
  ...
  ): Array<{ productId: string; quantity: number; unitPrice: number; subtotal: number; notes?: string }> {
  ...
  private async validateProductsExist(productIds: string[]): Promise<Map<string, Product>> {
    ...
    if (unavailable.length > 0) {
      throw new BadRequestException(`Products not available: ${unavailable.map(p => p.name).join(', ')}`);
    }
  ...
  private handleExceptions(error: unknown): never {
    if (error instanceof DatabaseError) {
      if (error.code === '23505') {
        ...
        throw new BadRequestException('An order with this information already exists');
      }
